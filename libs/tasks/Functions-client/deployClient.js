...
task("functions-deploy-client", "Deploys the ProjectFactory contract")
  .addOptionalParam("verify", "Set to true to verify client contract", false, types.boolean)
  .setAction(async (taskArgs) => {
    if (network.name === "hardhat") {
      throw Error(
        'This command cannot be used on a local hardhat chain.  Specify a valid network or simulate an ProjectFactory request locally with "npx hardhat functions-simulate".'
      )
    }

    console.log(`Deploying ProjectFactory contract to ${network.name}`)
    ...
    const clientContractFactory = await ethers.getContractFactory("Project")
    ...
    console.log(`\nProjectFactory contract deployed to ${clientContract.address} on ${network.name}`)
  })
